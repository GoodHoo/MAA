...
def train_multi_gan(args,
    ...
    # train_xes=[train_x.to(device) for train_x in train_xes]
    # train_y.to(device)
    # val_xes=[val_x.to(device) for val_x in val_xes]
    # val_y.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if backtrader:
            # 初始化一个空列表来存储所有生成器的验证指标
            all_val_metrics = []

            # 遍历所有生成器，计算财务指标
            for i in range(N):
                _,val_metrics = validate_financial_metric(generators[i], train_xes[i], train_y, val_xes[i], val_y, y_scaler)
                all_val_metrics.append(val_metrics)  # 保存每个生成器的结果

            # 将多个生成器的指标合并到日志信息中
            for i, val_metrics in enumerate(all_val_metrics):
                # 为每个生成器打印其对应的财务指标
                logging.info(
                    f"Generator {i + 1} | "
