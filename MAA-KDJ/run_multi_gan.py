...
def run_experiments(args):
    ...
    if args.N_pairs>1:
        ...
        if not args.ERM:
            results_file = os.path.join(args.output_dir, "gan_results.csv")
        else :
            results_file = os.path.join(args.output_dir, "erm_results.csv")
    ...
    raw_target_name = filename.replace('_processed', '').replace('processed_', '').replace('.csv', '')
    ...
    if args.mode == "train":
        financial_metric,results_acc = maa.train(logger)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--backtrader', type=bool, help="whether to do backtrader", default=False)
    ...
    parser.add_argument('--patience',  type=int, help="early stop patience")
