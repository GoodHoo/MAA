...
def run_experiments(args):
    ...
    if args.mode == "train":
        ...
        train_metrics, results_acc = maa.train(logger) # 解包返回值
        train_results, test_results = maa.pred()
        for i, result_acc in enumerate(results_acc):
            test_results[i]["Accuracy"] = round(results_acc[i] * 100, 2)
            test_results[i]["target"] = args.target_name
...
if __name__ == "__main__":
    ...
    # 修改了处理target_columns的方式，支持多个目标列
    parser.add_argument('--target_columns', nargs='+', type=int, help="target to be predicted", default=[4])
