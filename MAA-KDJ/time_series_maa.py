...
from trainer.multiGAN_backtrader_trainer import train_multi_gan
from trainer.baseframe_trainer import  train_baseframe
...
class MAA_time_series(MAABase):
    ...
    def init_dataloader(self):
        ...
        # 分别提取 x、y、y_gan 并堆叠
        self.train_x_all = [x for x, _, _, _ in train_data_list]
        self.train_y_all = train_data_list[0][1]  # 所有 y 应该相同，取第一个即可，不用cuda因为要eval
        self.train_y_gan_all = [y_gan for _, _, y_gan, _ in train_data_list]
        self.train_label_gan_all = [label_gan for _, _, _, label_gan in train_data_list]

        self.test_x_all = [x for x, _, _, _ in test_data_list]
        self.test_y_all = test_data_list[0][1]  # 所有 y 应该相同，取第一个即可，不用cuda因为要eval
        self.test_y_gan_all = [y_gan for _, _, y_gan, _ in test_data_list]
        self.test_label_gan_all = [label_gan for _, _, _, label_gan in test_data_list]
        ...
        if not (len(self.test_x_all[0]) == len(self.test_y_gan_all[0]) == len(self.test_label_gan_all[0])):
            raise ValueError(
                "self.test_x_all, self.test_y_gan_all, and self.test_label_gan_all must have the same length.")


        for i, (x, y_gan, label_gan) in enumerate(
                zip(self.test_x_all, self.test_y_gan_all, self.test_label_gan_all)):

            # 检查当前的数据集是否为空
            if len(x) == 0:  # 或者检查 y_gan 和 label_gan 的长度，如果它们应该总是相同的长度
                print(f"Warning: Skipping empty dataset at index {i}.")
                continue  # 跳过当前迭代

            dataloader = DataLoader(
                TensorDataset(x, y_gan, label_gan),
                batch_size=1,  # 保持 batch_size=1，但需要处理空数据集的情况
                generator=torch.manual_seed(self.seed),
                drop_last=False
            )
    ...
    def train(self, logger):
        if not self.ERM:
            financial_metric,best_acc,best_model_state = train_multi_gan(
                self.args, self.generators, self.discriminators, self.dataloaders,self.test_dataloaders,
                self.window_sizes,self.y_scaler,
            ...
            financial_metric,best_acc, best_model_state = train_baseframe(
                self.generators[0], self.dataloaders[0],self.test_dataloaders[0],
                self.y_scaler,
        ...
        return financial_metric,best_acc
    ...
    def pred(self):
        ...
        with torch.no_grad():
            for i, gen in enumerate(self.generators):
                ...
                # 前向预测（归一化后）
                y_pred,y_cls=gen(x_test)
                y_cls = F.softmax(y_cls, dim=-1).cpu().numpy()  # shape: (N, C)
                y_pred_norm = y_pred.cpu().numpy().reshape(-1, 1)  # (N,1)
                action=np.argmax(y_cls, axis=-1)
                confidence = y_cls[np.arange(y_cls.shape[0]),action]
                # 反归一化回原始值
